...
var swiper = new Swiper(".tarifs .swiper", {
  slidesPerView: "auto",
  spaceBetween: 14,
});
...
if(headerLang) {
  let activeLang = headerLang.children[0];
  let lagBox = headerLang.children[1]
  let langButtons = lagBox.children;
  activeLang.addEventListener("click", () => {
    if(lagBox.offsetHeight < 10) {
      lagBox.style.height = lagBox.scrollHeight + "px"
    }else {
      lagBox.style.height = "0"
    }
  })

  for(let i = 0; i < langButtons.length; i++) {
    if(langButtons[i].textContent === activeLang.children[0].textContent) {
      langButtons[i].style.display = "none"
    }
    langButtons[i].addEventListener("click", (e) => {
      lagBox.style.height = "0px"
      activeLang.children[0].textContent = e.target.textContent
    })
  }
}
